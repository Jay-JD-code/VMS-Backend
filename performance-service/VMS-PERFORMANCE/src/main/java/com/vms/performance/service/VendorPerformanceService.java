...
public class VendorPerformanceService {
    ...
    // ─────────────────────────────────────────────────────────────────
    // THRESHOLDS
    // Orders delivered within this many days are counted as "on time"
    ...
    public VendorPerformance calculatePerformance(String vendorId) {
        ...
        // Orders that were not cancelled — the "attempted" base
        List<OrderResponse> activeOrders = orders.stream()
                .filter(o -> !"CANCELLED".equals(o.getStatus()))
                .toList();

        // Orders that reached DELIVERED or COMPLETED
        ...
        // ── METRIC 1: DELIVERY SCORE (40% weight) ─────────────────────
        // On-time = reached DELIVERED/COMPLETED within ON_TIME_DAYS.
        // We prefer updatedAt (actual delivery timestamp) over now().
        long onTimeCount = deliveredOrders.stream()
                .filter(o -> {
                    if (o.getCreatedAt() == null) return false;
                    // Use updatedAt (when order reached DELIVERED/COMPLETED) if available;
                    // fall back to now() only when updatedAt is missing.
        ...
        double deliveryScore = deliveredOrders.isEmpty()
                ? 100.0  // no deliveries yet = no late deliveries = neutral 100
                : (onTimeCount * 100.0 / deliveredOrders.size());

        // ── METRIC 2: QUALITY SCORE (25% weight) ──────────────────────
        // Ratio of COMPLETED orders to all delivered orders.
        // An order that got stuck at DELIVERED and never completed counts against quality.
        double qualityScore = deliveredOrders.isEmpty()
                ? 100.0
                : (completedOrders.size() * 100.0 / deliveredOrders.size());

        // ── METRIC 3: FULFILLMENT SCORE (20% weight) ──────────────────
        // How many orders make it through without being cancelled.
        // High cancellation rate = unreliable vendor.
        double fulfillmentScore = totalOrders == 0
                ? 100.0
                : ((totalOrders - cancelledCount) * 100.0 / totalOrders);

        // ── METRIC 4: COMPLIANCE SCORE (15% weight) ───────────────────
        // Ratio of COMPLETED payments to total payments.
        // FAILED payments reduce the score proportionally.
        ...
        double complianceScore = totalPayments == 0
                ? 100.0  // no payments yet = no violations
        ...
        // Use updatedAt when available for accurate elapsed-time measurement.
